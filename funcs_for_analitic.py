...
def get_all_currency():
    ...
    while True:
        ...
        try:
          response = requests.get(f'https://cbr.ru/scripts/XML_daily.asp?date_req=01/{month_str}/{year}', timeout=10)
...
def process_data_need_vac():
    ...
    save_plot(salary_trend, 'Динамика уровня зарплат по годам', 'Год', 'Средняя зарплата (руб.)', 'dyn_salary_cities.png')
    ...
    save_plot(vacancy_trend, 'Динамика количества вакансий по годам', 'Год', 'Количество вакансий', 'cont_vacancy_cities.png')
    ...
    save_plot(salary_by_city, 'Уровень зарплат по городам', 'Город', 'Средняя зарплата (руб.)', 'level_sal_cities.png')
    ...
    save_plot(vacancy_share_by_city, 'Доля вакансий по городам', 'Город', 'Доля вакансий (%)', 'rate_vac_cities.png')
    ...
    for year, skill in skill_trends.items():
        save_plot(skill, 'ТОП-20 навыков по годам', 'Навык', 'Количество навыков', f'top_20_skills_{year}.png')
